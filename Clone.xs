...
av_clone_iterative (pTHX_ SV* ref, HV* hseen, int rdepth, AV* weakrefs) {
    ...
    if ( av_len(self) == 0 ) {
        ...
        if ( elem && SvROK(*elem) && SvTYPE(SvRV(*elem)) == SVt_PVAV ) {
            ...
            while ( current && SvROK(current) && SvTYPE( SvRV(current) ) == SVt_PVAV && av_len( (AV*)SvRV(current) ) == 0 ) {
                AV *new_av = newAV();
...
static SV*
av_clone (pTHX_ SV* ref, SV* target, HV* hseen, int depth, int rdepth, AV* weakrefs)
{
...
/*
static SV *
rv_clone (pTHX_ SV * ref, HV* hseen, int depth, int rdepth, AV * weakrefs) {
    SV *clone = NULL;
...
static SV*
sv_clone (pTHX_ SV* ref, HV* hseen, int depth, int rdepth, AV* weakrefs) {
     ...
     * Copy through magic to get a plain unshared value. */
    if ( SvTYPE(ref) == SVt_PVLV && SvMAGICAL(ref) ) {
        MAGIC *mg;
        for ( mg = SvMAGIC(ref); mg; mg = mg->mg_moremagic ) {
            if ( (mg->mg_type == PERL_MAGIC_tiedelem || mg->mg_type == PERL_MAGIC_tiedscalar)
                && is_threads_shared_tie(mg->mg_obj) ) {
                TRACEME(("threads::shared tiedelem PVLV — copy value\n"));
                clone = newSVsv(ref);
                if ( visible && ref != clone ) CLONE_STORE(ref, clone);
                return clone;
            }
        }
    }
    ...
    if (SvMAGICAL(ref)) {
        ...
        for ( mg = SvMAGIC(ref); mg; mg = mg->mg_moremagic ) {
            ...
            if ( mg->mg_type == '~' ) {
                ...
                if ( mg->mg_virtual && mg->mg_virtual->svt_dup && (mg->mg_flags & MGf_DUP) ) {
                    MAGIC *new_mg;
                    new_mg = sv_magicext( clone, mg->mg_obj, mg->mg_type, mg->mg_virtual, mg->mg_ptr, mg->mg_len );
             ...
             * the tie to read the actual data. */
            if ( mg->mg_type == PERL_MAGIC_shared_scalar || mg->mg_type == PERL_MAGIC_shared )
                continue;
            ...
            if ( mg->mg_obj != NULL ) {
                switch ( mg->mg_type ) {
                    ...
                    case 'q': /* PERL_MAGIC_tiedscalar */
                        ...
                        if ( is_threads_shared_tie(mg->mg_obj) ) continue;
                        magic_ref++;
                    /* fall through */
                    default:
                        obj = sv_clone( aTHX_ mg->mg_obj, hseen, -1, rdepth, weakrefs ); 
                }
            }
            else {
